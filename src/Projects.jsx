...
function ProjectCard({ title, demo, repo, desc }) {
    ...
    return (
        <div className="w-68 h-60 perspective"
            onMouseEnter={()=>setIsHover(true)}
            onMouseLeave={()=>setIsHover(false)}
        >
            
            <motion.div className="relative w-full h-full preserve-3d cursor-pointer"
            animate={{ rotateY: isHover ? 180 : 0 }}
            transition={{ duration: 0.6, ease: "easeInOut" }}
            >
            
            <div className="absolute w-full h-full bg-[#1A1A1C] p-8 rounded-lg flex flex-col gap-4 items-center justify-center text-center text-white backface-hidden">
                <h6 className="text-[#FFD700] text-2xl sm:text-2xl md:text-2xl lg:text-2xl">{title}</h6>
                <p>{desc}</p>
                
            </div>

            <div className="absolute w-full h-full bg-[#FFD700] p-8 rounded-lg flex flex-col gap-3 items-center justify-center text-center text-black backface-hidden rotate-y-180">
